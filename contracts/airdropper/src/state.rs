...
/// Map that stores the `(collection_id, token_id)`s promised to an address
/// This map will get modified as addresses claim their promises or if
/// a maintainer pushes the tokens to an address
pub const ADDRESS_PROMISED_TOKEN_IDS: Map<Addr, Vec<(u64, u32)>> =
    Map::new("address_promised_token_ids");
/// Map that stores an address' claimed `(collection_id, token_id)`s
/// dependent on the `ADDRESS_PROMISED_TOKEN_IDS` map
pub const ADDRESS_CLAIMED_TOKEN_IDS: Map<Addr, Vec<(u64, u32)>> =
    Map::new("address_claimed_token_ids");
/// Helper map that holds what address a `(collection_id, token_id)` is assigned to
pub const ASSIGNED_TOKEN_IDS: Map<(u64, u32), Addr> = Map::new("assigned_token_ids");
/// Helper map that holds what address a `(collection_id, token_id)` has been claimed by
pub const CLAIMED_TOKEN_IDS: Map<(u64, u32), Addr> = Map::new("claimed_token_ids");
